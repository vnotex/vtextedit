...
Searcher::Searcher(InputModeManager *manager)
    ...
    , m_interface(manager->editorInterface())
{
}
...
Range Searcher::findPatternForMotion(const SearchParams& searchParams, const KateViI::Cursor &startFrom, int count) const
{
...
Range Searcher::findWordForMotion(const QString &word, bool backwards, const KateViI::Cursor &startFrom, int count)
{
...
KateViI::Range Searcher::findPattern(const SearchParams& searchParams, const KateViI::Cursor &startFrom, int count, bool addToSearchHistory)
{
...
KateViI::Range Searcher::findPatternWorker(const SearchParams& searchParams, const KateViI::Cursor &startFrom, int count) const
{
    ...
    for (int i = 0; i < count; i++) {
        if (!searchParams.isBackwards) {
            const KateViI::Range matchRange = m_interface->searchText(KateViI::Range(KateViI::Cursor(searchBegin.line(), searchBegin.column() + 1), m_interface->documentEnd()), pattern, flags).first();

            if (matchRange.isValid()) {
                finalMatch = matchRange;
            } else {
                // Wrap around.
                const KateViI::Range wrappedMatchRange = m_interface->searchText(KateViI::Range(m_interface->documentRange().start(), m_interface->documentEnd()), pattern, flags).first();
            ...
            KateViI::Cursor newSearchBegin = KateViI::Cursor(searchBegin.line(), m_interface->lineLength(searchBegin.line()));
            KateViI::Range bestMatch = KateViI::Range::invalid();
            while (true) {
                QVector<KateViI::Range> matchesUnfiltered = m_interface->searchText(KateViI::Range(newSearchBegin, m_interface->documentRange().start()), pattern, flags);
            ...
            if (matchRange.isValid()) {
                ...
                const KateViI::Range wrappedMatchRange = m_interface->searchText(KateViI::Range(m_interface->documentEnd(), m_interface->documentRange().start()), pattern, flags).first();

...
VisualViMode::VisualViMode(InputModeManager *viInputModeManager,
  ...
  m_interface->connectSelectionChanged([this]() { updateSelection(); });
}

VisualViMode::~VisualViMode() {}

void VisualViMode::selectInclusive(const KateViI::Cursor &c1, const KateViI::Cursor &c2) {
  if (c1 >= c2) {
    m_interface->setSelection(KateViI::Range(c1.line(), c1.column() + 1, c2.line(), c2.column()));
  } else {
    m_interface->setSelection(KateViI::Range(c1.line(), c1.column(), c2.line(), c2.column() + 1));
  }
}

void VisualViMode::selectBlockInclusive(const KateViI::Cursor &c1, const KateViI::Cursor &c2) {
...
void VisualViMode::selectLines(const KateViI::Range &range) {
  ...
  m_interface->setSelection(
      KateViI::Range(KateViI::Cursor(sline, 0), KateViI::Cursor(eline, ecol)));
}
...
void VisualViMode::goToPos(const Range &r) {
  ...
  } else if (r.startLine != -1 && r.startColumn != -1 &&
...
void VisualViMode::setVisualModeType(ViMode mode) {
  Q_ASSERT(mode == ViMode::VisualMode || mode == ViMode::VisualLineMode ||

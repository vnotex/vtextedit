...
KeyParser::KeyParser() { initKeyTables(); }
  ...
  for (int i = 0; i < keys.length(); i++) {
    ...
    if (insideTag) {
      if (c == QLatin1Char('>')) {
        ...
        // contains modifiers
        if (keys.mid(i).indexOf(QLatin1Char('-')) != -1 &&
            keys.mid(i).indexOf(QLatin1Char('-')) < keys.mid(i).indexOf(QLatin1Char('>'))) {
          // Perform something similar to a split on '-', except that we want to
          // keep the occurrences of '-' e.g. <c-s-a> will equate to the list of
          // tokens "c-", "s-", "a". A straight split on '-' would give us "c",
          // "s", "a", in which case we lose the piece of information that 'a'
          // is just the 'a' key, not the 'alt' modifier.
          ...
          for (const QString &str : qAsConst(tokens)) {
            if (str == QLatin1String("s-") && (keyCodeTemp & 0x01) != 0x1) {
              ...
              if (m_nameToKeyCode.contains(str) ||
                  (str.length() == 1 && str.at(0).isLetterOrNumber())) {
                if (m_nameToKeyCode.contains(str)) {
          ...
          if (keys.indexOf(QLatin1Char('>'), i) != -1 &&
              (m_nameToKeyCode.contains(str) ||
               (str.length() == 1 && str.at(0).isLetterOrNumber()))) {
            if (m_nameToKeyCode.contains(str)) {
              keyCodeTemp += m_nameToKeyCode.value(str) * 0x10;
            } else {
      ...
      if (c == QLatin1Char('<')) {
        // If there's no closing '>', or if there is an opening '<' before the
        // next '>', interpret as a literal '<' If we are <space>, encode as a
        // literal " ".
        const QString rest = keys.mid(i);
        if (rest.indexOf(QLatin1Char('>'), 1) != -1 &&
            rest.mid(1, rest.indexOf(QLatin1Char('>'), 1) - 1) == QLatin1String("space")) {
          encodedSequence.append(QLatin1Char(' '));
          i += rest.indexOf(QLatin1Char('>'), 1);
          continue;
        } else if (rest.indexOf(QLatin1Char('>'), 1) == -1 ||
                   (rest.indexOf(QLatin1Char('<'), 1) < rest.indexOf(QLatin1Char('>'), 1) &&
                    rest.indexOf(QLatin1Char('<'), 1) != -1)) {

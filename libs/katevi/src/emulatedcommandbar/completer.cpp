...
Completer::Completer(EmulatedCommandBar *emulatedCommandBar,
                     KateViI::KateViEditorInterface *editorInterface, QLineEdit *edit)
    : m_edit(edit), m_interface(editorInterface) {
  m_completer = new QCompleter(QStringList(), edit);
  // Can't find a way to stop the QCompleter from auto-completing when attached
  // to a QLineEdit, so don't actually set it as the QLineEdit's completer.
...
bool Completer::completerHandledKeyPress(const QKeyEvent *keyEvent) {
  ...
  if ((modifiers == Qt::ControlModifier && key == Qt::Key_P) || key == Qt::Key_Down) {
    if (!m_completer->popup()->isVisible()) {
      const auto completionStartParams =
  ...
  if ((modifiers == Qt::ControlModifier && key == Qt::Key_N) || key == Qt::Key_Up) {
    if (!m_completer->popup()->isVisible()) {
      const auto completionStartParams =
...
void Completer::setCurrentMode(ActiveMode *currentMode) { m_currentMode = currentMode; }
...
CompletionStartParams Completer::activateWordFromDocumentCompletion() {
  ...
  QSet<QString> set;
  // Narrow the range of lines we search around the cursor so that we don't die
  // on huge files.
...
int Completer::wordBeforeCursorBegin() {
  ...
  while (wordBeforeCursorBegin >= 0 && (text[wordBeforeCursorBegin].isLetterOrNumber() ||

#ifndef KATEVII_COMMAND_H
#define KATEVII_COMMAND_H
 ...
 *
 * In case the Command needs to interactively process the text of the
 * parameters, override wantsToProcessText() by returning @e true and
 * reimplement processText().
 *
 ...
 */
class KATEVI_EXPORT Command : public QObject {
  Q_OBJECT

public:
  /**
   * Constructor with \p parent.
   * Will register this command for the commands names given in \p cmds at the
   * global editor instance.
   */
  Command(const QStringList &cmds, QObject *parent = nullptr) : QObject(parent), m_cmds(cmds) {}
...
public:
   ...
   */
  inline const QStringList &cmds() const { return m_cmds; }
   ...
   */
  virtual bool supportsRange(const QString &cmd) { return false; }
   ...
   *
   * If a non-invalid range is given, the command shall be executed on that
   * range. supportsRange() tells if the command supports that.
   *
   * \return \e true on success, otherwise \e false
   */
  virtual bool exec(KateViEditorInterface *editorInterface, const QString &cmd, QString &msg,
                    const KateViI::Range &range = KateViI::Range::invalid()) = 0;
   ...
   */
  virtual bool help(KateViEditorInterface *editorInterface, const QString &cmd, QString &msg) = 0;
   ...
   * The returned completion object is deleted automatically once it is not
   * needed anymore. Therefore neither delete the completion object yourself nor
   * return the same completion object twice.
   *
   ...
   *         not supported
   */
  virtual KCompletion *completionObject(KateViEditorInterface *editorInterface,
   ...
   */
  virtual void processText(KateViEditorInterface *editorInterface, const QString &text) {}

...
bool NormalViMode::handleKeyPress(const QKeyEvent *e)
{
    ...
    if (keyCode == Qt::Key_Escape
        ...
        // still be in insert mode, but behaves as if it's in normal mode. I'm treating
        // the status bar thing as a bug and just exiting insert mode altogether.
    ...
    if ((keyCode >= Qt::Key_0 && keyCode <= Qt::Key_9 && lastChar != QLatin1Char('"')) // key 0-9
        ...
        && (!isWaitingForRegisterOrChar) // Not in the middle of "find char" motions or replacing char.
    ...
    */
    ...
    const int checkFrom = m_awaitingMotionOrTextObject.isEmpty() ? 0
                                                                 : m_awaitingMotionOrTextObject.top();
    ...
    if (m_matchingCommands.size() == 1) {
        ...
        if (m_commands.at(cmdIdx)->matchesExact(m_keys)
            && !m_commands.at(cmdIdx)->needsMotion()) {
            if (m_viInputModeManager->getCurrentViMode() == ViMode::NormalMode) {
...
void NormalViMode::resetParser()
{
    ...
    if (viMode == ViMode::NormalMode
        || viMode == ViMode::VisualLineMode
        || viMode == ViMode::VisualBlockMode
        || viMode == ViMode::VisualMode) {
        m_viInputModeManager->inputAdapter()->setCaretStyle(KateViI::Block);
...
void NormalViMode::executeCommand(const Command *cmd)
{
    ...
    if (newViMode != ViMode::InsertMode && newViMode != ViMode::ReplaceMode) {
        ...
        const bool commandSwitchedToVisualMode = (originalViMode == ViMode::NormalMode)
                                                 && m_viInputModeManager->isAnyVisualMode();
...
bool NormalViMode::commandDeleteToEOL()
{
    ...
    case LineWise:
        ...
        c.setColumn(getFirstNonBlank(qMin(m_interface->lastLine(),
...
bool NormalViMode::commandChangeCaseRange()
{
    ...
    KateViI::Range range = KateViI::Range(m_commandRange.startLine,
...
bool NormalViMode::commandChangeLine()
{
    ...
    Range r(c.line(), c.column(),
            c.line(), m_interface->lineLength(c.line()) - 1,
            InclusiveMotion);
...
{
    return executeEditorCommand(QStringLiteral("split"));
}

bool NormalViMode::commandSplitVert()
{
    return executeEditorCommand(QStringLiteral("vsplit"));
}

bool NormalViMode::commandCloseView()
{
    return executeEditorCommand(QStringLiteral("close"));
}
...
{
    return executeEditorCommand(QStringLiteral("q!"));
}

bool NormalViMode::commandCloseWrite()
{
    return executeEditorCommand(QStringLiteral("wq"));
}
...
Range NormalViMode::textObjectAWord()
{
    ...
    if (c2.line() != c.line()
        && c2.column() == m_interface->lineLength(c2.line()) - 1) {
    ...
    const bool swallowPrecedingSpaces = (c2.column() == m_interface->lineLength(c2.line()) - 1
                                        ...
                                        || startedOnSpace
                                        || swallowCarriageReturnAtEndOfLine;
...
Range NormalViMode::textObjectInnerCurlyBracket()
{
    ...
    if (innerCurlyBracket.startLine != innerCurlyBracket.endLine) {
        const bool openingBraceIsLastCharOnLine = innerCurlyBracket.startColumn == m_interface->line(innerCurlyBracket.startLine).length();
        ...
        const QString textLeadingClosingBracket = m_interface->line(innerCurlyBracket.endLine).mid(0, innerCurlyBracket.endColumn + 1);
...
void NormalViMode::initializeCommands()
{
    ...
    */
    ...
    // TODO: Different from Vi: Y should yank lines instead of yank to the end of the line.
...
bool NormalViMode::paste(PasteLocation pasteLocation, bool isgPaste, bool isIndentedPaste)
{
    ...
    if (m == LineWise) {
        ...
        if (isIndentedPaste) {
            ...
            const QString leadingWhiteSpaceOnCurrentLine = lineText.mid(0, lineText.indexOf(QRegularExpression(QLatin1String("[^\\s]"))));
            const QString leadingWhiteSpaceOnFirstPastedLine = textToInsert.mid(0, textToInsert.indexOf(QRegularExpression(QLatin1String("[^\\s]"))));
        ...
        if (pasteLocation == AfterCurrentPosition) {
            ...
            pasteAt.setColumn(m_interface->lineLength(pasteAt.line()));     // paste after the current line and ...
...
KateViI::Cursor NormalViMode::cursorPosAtEndOfPaste(const KateViI::Cursor &pasteLocation, const QString &pastedText) const
{
...
Range NormalViMode::textObjectComma(bool inner) const
{
    ...
    Range r(0, 0,
            m_interface->lines(), m_interface->line(m_interface->lastLine()).length(),
            InclusiveMotion);
...
void NormalViMode::highlightYank(const Range &range, const OperationMode mode)
{
    ...
    if (mode == Block) {
        ...
        addHighlightYank(KateViI::Range(range.startLine, range.startColumn, range.endLine, range.endColumn));
    }
}

void NormalViMode::addHighlightYank(const KateViI::Range &yankRange)
{
    KATEVI_NIY;
}
...
void NormalViMode::rewriteKeysInCWCase()
{
    ...
    const KateViI::Cursor endOfWordOrWORD = isWORD ? findWORDEnd(currentPosition.line(), currentPosition.column() - 1, true)
...
void NormalViMode::executeMotionWithoutCommand(Motion *p_motion)
{
    ...
    if (r.valid
        && r.endLine >= 0
        && (r.endLine == 0 || r.endLine <= m_interface->lines() - 1)
        && r.endColumn >= 0) {

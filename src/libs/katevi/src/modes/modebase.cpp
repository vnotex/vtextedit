...
KateViI::Cursor ModeBase::findNextWordStart(int fromLine, int fromColumn, bool onlyCurrentLine) const
{
...
KateViI::Cursor ModeBase::findPrevWordEnd(int fromLine, int fromColumn, bool onlyCurrentLine) const
{
    QString line = getLine(fromLine);
...
KateViI::Cursor ModeBase::findPrevWordStart(int fromLine, int fromColumn, bool onlyCurrentLine) const
{
    QString line = getLine(fromLine);
...
{
    qDebug() << "Not implemented yet" << errorMsg;
}

void ModeBase::message(const QString &msg)
{
    qDebug() << "Not implemented yet" << msg;
}
...
void ModeBase::addToNumberUnderCursor(int count)
{
    ...
    m_interface->editStart();
    m_interface->removeText(KateViI::Range(c.line(), numberStartPos, c.line(), numberStartPos + numberAsString.length()));
    m_interface->insertText(KateViI::Cursor(c.line(), numberStartPos), newNumberText);
    m_interface->editEnd();
    updateCursor(KateViI::Cursor(m_interface->cursorPosition().line(), numberStartPos + newNumberText.length() - 1));
}

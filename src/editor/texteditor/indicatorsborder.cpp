      ...
      m_enableTextFolding(p_enableTextFolding)
{
    ...
    connect(&m_foldingHighlightTimer, &QTimer::timeout,
            this, &IndicatorsBorder::highlightFolding);
}
...
{
    return QSize(borderWidth(), 0);
}

const QFont &IndicatorsBorder::getFont() const
{
    return m_font;
}
...
void IndicatorsBorder::setForegroundColor(const QColor &p_color)
{
    m_foregroundColor = p_color;
}

void IndicatorsBorder::setBackgroundColor(const QColor &p_color)
{
...
void IndicatorsBorder::setFoldingColor(const QColor &p_color)
{
    m_foldingColor = p_color;
}
...
void IndicatorsBorder::paintBorder(const QRect &p_rect)
{
    ...
    while (block.isValid() && rect.y() <= bottom) {
        ...
        if (m_lineNumberType != LineNumberType::None) {
            ...
            painter.drawText(x + m_maxCharWidth / 2,
                             y,
        ...
        if (m_enableTextFolding) {
            ...
            if (!foldingRanges.isEmpty()
                || TextBlockData::get(block)->isMarkedAsFoldingStart()) {
...
void IndicatorsBorder::updateBorder()
{
    update();
}
...
void IndicatorsBorder::paintFoldingMarker(QPainter &p_painter,
                                          int p_xOffset,
                                          int p_yOffset,
                                          int p_width,
                                          int p_height,
                                          bool p_folded) const
{
    ...
    p_painter.drawText(p_xOffset,
                       p_yOffset,
                       p_width,
                       p_height,
                       Qt::TextDontClip | Qt::AlignCenter | Qt::AlignVCenter,
                       markerText);
...
void IndicatorsBorder::mousePressEvent(QMouseEvent *p_event)
{
    ...
    if (block.isValid()) {
        ...
        if (indicator == Indicators::LineNumber
            && p_event->button() == Qt::LeftButton
            && !(p_event->modifiers() & Qt::ShiftModifier)) {
            // TODO: Let view begin to select lines.
        }

        QMouseEvent forwardEvent(QEvent::MouseButtonPress,
                                 QPoint(0, p_event->y()),
                                 p_event->button(),
                                 p_event->buttons(),
                                 p_event->modifiers());
...
void IndicatorsBorder::mouseReleaseEvent(QMouseEvent *p_event)
{
    ...
    QMouseEvent forwardEvent(QEvent::MouseButtonRelease,
                             QPoint(0, p_event->y()),
                             button,
                             p_event->buttons(),
                             p_event->modifiers());
...
void IndicatorsBorder::highlightFolding()
{
    ...
    if (!newFoldingRange) {
        ...
        const int searchEndBlockNum = qMax(0,
            m_currentFoldingHighlightBlockNumber - c_maxNumberOfLinesToSearchBackwardsForSyntaxFolding);
        for (int blockNum = m_currentFoldingHighlightBlockNumber;
             blockNum >= searchEndBlockNum;
             --blockNum) {

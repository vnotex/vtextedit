...
QTextCursor EditorInputMode::textCursor() const
{
...
void EditorInputMode::connectTextInserted(std::function<void(const KateViI::Range &)> p_slot)
{
    connect(document(), &QTextDocument::contentsChange,
            this, [this, p_slot](int p_position, int p_charsRemoved, int p_charsAdded) {
                      if (p_charsAdded > 0) {
...
void EditorInputMode::setSelection(const KateViI::Range &p_range)
{
    ...
    // For KateVi, we need to make the selection without changing the cursor position.
    auto cursor = textCursor();
...
int EditorInputMode::lines() const
{
    return document()->blockCount();
}

void EditorInputMode::removeSelection()
{
    clearSelection();
}
...
KateViI::Range EditorInputMode::selectionRange() const
{
    if (m_textEdit->hasSelection()) {
        ...
        return KateViI::Range(toKateViCursor(selection.start()),
                              toKateViCursor(selection.end()));
    }
...
bool EditorInputMode::isUndoMergeAllEditsEnabled() const
{
    return m_mergeAllEditsEnabled;
}
...
QTextDocument *EditorInputMode::document() const
{
...
KateViI::Cursor EditorInputMode::toKateViCursor(int p_position) const
{
    ...
    return KateViI::Cursor(block.blockNumber(),
                           p_position - block.position());
}
...
int EditorInputMode::undoCount() const
{
    return document()->isUndoAvailable() ? 1 : 0;
}

void EditorInputMode::undo()
{
    m_textEdit->undo();
}

int EditorInputMode::redoCount() const
{
    return document()->isRedoAvailable() ? 1 : 0;
}

void EditorInputMode::redo()
{
    m_textEdit->redo();
}

int EditorInputMode::lastLine() const
{
    return lines() - 1;
}

bool EditorInputMode::replaceText(const KateViI::Range &p_range, const QString &p_text, bool p_blockWise)
{
    if (p_blockWise) {
        EDITOR_NIY;
    } else {
...
bool EditorInputMode::selection() const
{
    return m_textEdit->hasSelection();
}

bool EditorInputMode::insertText(const KateViI::Cursor &p_position, const QString &p_text, bool p_blockWise)
{
    if (m_editor->isReadOnly()) {
        return false;
    }
...
void EditorInputMode::connectMarkChanged(std::function<void(KateViI::KateViEditorInterface *p_editorInterface,
                                         KateViI::Mark p_mark,
                                         KateViI::MarkInterface::MarkChangeAction p_action)> p_slot)
{
    EDITOR_NIY;
}

void EditorInputMode::removeMark(int line, uint markType)
{
    EDITOR_NIY;
}
...
void EditorInputMode::clearOverriddenSelection()
{
    m_textEdit->clearOverriddenSelection();
}
...
void EditorInputMode::abortCompletion()
{
    m_editor->abortCompletion();
}
...
void EditorInputMode::setOverwriteMode(bool p_enabled)
{
    m_textEdit->setOverwriteMode(p_enabled);
}

QWidget *EditorInputMode::focusProxy() const
{
    return m_textEdit;
}

bool EditorInputMode::isCompletionActive() const
{
    return m_editor->isCompletionActive();
}

void EditorInputMode::completionNext(bool p_reversed)
{
    m_editor->completionNext(p_reversed);
}
...
void EditorInputMode::completionExecute()
{
    m_editor->completionExecute();
}

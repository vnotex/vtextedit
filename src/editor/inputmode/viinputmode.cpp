...
ViStatusBar::ViStatusBar(KateVi::EmulatedCommandBar *p_commandBar)
    : m_commandBar(p_commandBar)
{
    ...
    connect(m_commandBar, &KateVi::EmulatedCommandBar::hideMe,
            this, [this]() {
                m_commandBar->closed();
                m_commandBar->hide();
                emit commandBarHidden();
            });
    connect(m_commandBar, &KateVi::EmulatedCommandBar::showMe,
            this, [this]() {
                m_commandBar->show();
...
void ViStatusBar::setKeyStroke(const QString &p_keys)
{
    m_keyStrokeIndicator->setText(p_keys);
}

ViStatusWidget::ViStatusWidget(const QSharedPointer<ViStatusBar> &p_widget)
    : InputModeStatusWidget(nullptr),
      m_statusBar(p_widget)
{
    connect(m_statusBar.data(), &ViStatusBar::commandBarHidden,
            this, &InputModeStatusWidget::focusOut);
    connect(m_statusBar.data(), &ViStatusBar::commandBarShown,
            this, &InputModeStatusWidget::focusIn);
}

QSharedPointer<QWidget> ViStatusWidget::widget()
{
    return m_statusBar;
}
...
QString ViInputMode::name() const
{
    return QStringLiteral("vi");
}

InputMode ViInputMode::mode() const
{
...
void ViInputMode::showViModeEmulatedCommandBar()
{
    emit m_viEmulatedCommandBar->showMe();
}

KateVi::InputModeManager *ViInputMode::viInputModeManager() const
{
    return m_viModeManager.data();
}
...
bool ViInputMode::isActive() const
{
    return m_active;
}
...
int ViInputMode::linesDisplayed()
{
    return m_interface->linesDisplayed();
}

void ViInputMode::focusIn()
{
    updateCursorBlinking();
}

void ViInputMode::focusOut()
{
    restoreCursorBlinking();
}
...
bool ViInputMode::keyPress(QKeyEvent *p_event)
{
    return handleKeyPress(p_event);
}

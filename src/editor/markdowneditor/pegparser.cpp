...
QVector<ElementRegion> PegParser::parseImageRegions(const QSharedPointer<PegParseConfig> &p_config)
{
...
pmh_element **PegParser::parseMarkdownToElements(const QSharedPointer<PegParseConfig> &p_config)
{
    ...
    // returning 2. pmh_markdown_to_elements() will treat it at the size of 1 (expected).
...
int PegParser::getNumberOfStyles()
{
    return pmh_NUM_LANG_TYPES;
}

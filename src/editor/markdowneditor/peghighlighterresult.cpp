...
void PegHighlighterResult::parseBlocksHighlights(QVector<QVector<peg::HLUnit>> &p_blocksHighlights,
                                                 ...
                                                 const QSharedPointer<peg::PegParseResult> &p_result)
{
    ...
    for (int i = 0; i < p_blocksHighlights.size(); ++i) {
        if (p_blocksHighlights[i].size() > 1) {
            std::sort(p_blocksHighlights[i].begin(),
                      p_blocksHighlights[i].end(),
                      peg::HLUnitLess());
        }
    }
}

void PegHighlighterResult::parseBlocksHighlightOne(QVector<QVector<peg::HLUnit>> &p_blocksHighlights,
...
const QVector<peg::HLUnitStyle> &PegHighlighterResult::getCodeBlockHighlight(int p_blockNumber) const
{
...
void PegHighlighterResult::setCodeBlockHighlights(int p_index,

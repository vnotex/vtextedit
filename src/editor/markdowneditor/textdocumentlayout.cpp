...
TextDocumentLayout::TextDocumentLayout(QTextDocument *p_doc,
...
void TextDocumentLayout::blockRangeFromRect(const QRectF &p_rect,
                                             int &p_first,
                                             int &p_last) const
{
    if (p_rect.isNull()) {
...
void TextDocumentLayout::blockRangeFromRectBS(const QRectF &p_rect,
                                              int &p_first,
                                              int &p_last) const
{
    if (p_rect.isNull()) {
...
QVector<QTextLayout::FormatRange> TextDocumentLayout::formatRangeFromSelection(const QTextBlock &p_block,
...
QRectF TextDocumentLayout::frameBoundingRect(QTextFrame *p_frame) const
{
    ...
    return QRectF(0,
                  0,
                  qMax(document()->pageSize().width(), m_width),
                  qreal(INT_MAX));
}
...
qreal TextDocumentLayout::layoutLines(const QTextBlock &p_block,
                                      QTextLayout *p_tl,
                                      ...
                                      qreal p_height)
{
    ...
    while (true) {
        ...
        if (pPreviewData) {
            ...
            qreal imgHeight = fetchInlineImagesForOneLine(*pPreviewData,
                                                          &line,
                                                          m_margin,
                                                          imgIdx,
                                                          images,
...
void TextDocumentLayout::layoutInlineImage(const PreviewImageData *p_data,
...
void TextDocumentLayout::drawPreview(QPainter *p_painter,
...
void TextDocumentLayout::drawPreviewMarker(QPainter *p_painter,
                                           const QTextBlock &p_block,
                                           const QPointF &p_offset)
{
...
int TextDocumentLayout::getTextWidthWithinTextLine(const QTextLayout *p_layout,
                                                   int p_pos,
                                                   int p_length)
{
...
qreal TextDocumentLayout::getLeadingSpaceOfLine() const
{
    return m_leadingSpaceOfLine;
}

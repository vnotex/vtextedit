...
namespace vte
{
    // When a TextBlockRange is invalid, its first and last block number is not reliable.
    ...
    public:
        ...
        bool isValid() const
        {
            if (m_first.isValid() && m_last.isValid()) {
                ...
                int d = l - f;
                if (d >= 0
                    && !(d < m_validLastBlockNumber - m_validFirstBlockNumber
                         && f < m_validFirstBlockNumber)) {
                    return true;
                }
            }
        ...
        bool contains(int p_blockNumber) const
        {
            ...
            return m_first.blockNumber() <= p_blockNumber
                   && m_last.blockNumber() >= p_blockNumber;
        }
        ...
        const QTextBlock &first() const
        {
            return m_first;
        }

        const QTextBlock &last() const
        {
            return m_last;
        }

#ifndef TEXTEDITINPUTMODE_H
#define TEXTEDITINPUTMODE_H
...
// Editor interface implementation that only depends on a VTextEdit.
// Editors providing extra facilities (text folding, completion, ...) may
// derive from this and override the corresponding virtuals.
class TextEditInputMode : public QObject, public InputModeEditorInterface {
  Q_OBJECT
public:
  explicit TextEditInputMode(VTextEdit *p_textEdit);

signals:
  // Emitted from notifyEditorModeChanged() when the mode really changes.
  void editorModeChanged();

  // Focus signals of the underlying VTextEdit.
  void textEditFocusIn();

  void textEditFocusOut();

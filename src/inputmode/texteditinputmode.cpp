...
TextEditInputMode::TextEditInputMode(VTextEdit *p_textEdit) : m_textEdit(p_textEdit) {
  Q_ASSERT(m_textEdit);
  // Observe-only filter, used to publish focus signals.
  m_textEdit->installEventFilter(this);
}

bool TextEditInputMode::eventFilter(QObject *p_obj, QEvent *p_event) {
  if (p_obj == m_textEdit) {
    if (p_event->type() == QEvent::FocusIn) {
      emit textEditFocusIn();
    } else if (p_event->type() == QEvent::FocusOut) {
      emit textEditFocusOut();
    }
  }
...
QTextCursor TextEditInputMode::textCursor() const { return m_textEdit->textCursor(); }

void TextEditInputMode::setCaretStyle(CaretStyle p_style) {
...
void TextEditInputMode::clearSelection() {
...
void TextEditInputMode::notifyEditorModeChanged(EditorMode p_mode) {
  if (m_mode != p_mode) {
    m_mode = p_mode;
...
void TextEditInputMode::copyToClipboard(const QString &p_text) {
...
void TextEditInputMode::editStart() {
...
void TextEditInputMode::editEnd() {
  --m_editSessionCount;
...
bool TextEditInputMode::removeLine(int p_line) {
...
void TextEditInputMode::connectSelectionChanged(std::function<void()> p_slot) {
  connect(m_textEdit, &QTextEdit::selectionChanged, this, p_slot);
}

void TextEditInputMode::connectTextInserted(std::function<void(const KateViI::Range &)> p_slot) {
...
void TextEditInputMode::setSelection(int p_startLine, int p_startColumn, int p_endLine,
                                     int p_endColumn) {
  clearSelection();
  auto cursor = textCursor();
...
void TextEditInputMode::setSelection(const KateViI::Range &p_range) {
  if (!p_range.isValid()) {
    clearSelection();
    return;
  }
...
void TextEditInputMode::setBlockSelection(bool p_enabled) {
  EDITOR_NIY;
  Q_ASSERT(!p_enabled);
}

int TextEditInputMode::lineLength(int p_line) const {
  auto block = document()->findBlockByNumber(p_line);
  return block.length() - 1;
}

KateViI::Cursor TextEditInputMode::cursorPosition() const {
  auto cursor = textCursor();
  return KateViI::Cursor(cursor.block().blockNumber(), cursor.positionInBlock());
}

int TextEditInputMode::lines() const { return document()->blockCount(); }

void TextEditInputMode::removeSelection() { clearSelection(); }

KateViI::Range TextEditInputMode::selectionRange() const {
...
KateViI::Range TextEditInputMode::documentRange() const {
  return KateViI::Range(KateViI::Cursor::start(), documentEnd());
}

void TextEditInputMode::setUndoMergeAllEdits(bool p_merge) {
...
bool TextEditInputMode::isUndoMergeAllEditsEnabled() const { return m_mergeAllEditsEnabled; }

void TextEditInputMode::notifyViewModeChanged(KateViI::ViewMode p_mode) {
  notifyEditorModeChanged(static_cast<EditorMode>(p_mode));
}

KateViI::ViewMode TextEditInputMode::viewMode() const {
  auto mode = m_textEdit->getInputMode();
  if (!mode) {
    // The input mode is not installed yet (during its construction).
    return static_cast<KateViI::ViewMode>(m_mode);
  }
  return static_cast<KateViI::ViewMode>(mode->editorMode());
}

void TextEditInputMode::cursorPrevChar(bool p_selection) {
  auto cursor = textCursor();
...
void TextEditInputMode::update() { m_textEdit->update(); }

QString TextEditInputMode::currentTextLine() const {
  auto cursor = textCursor();
  return cursor.block().text();
}

QString TextEditInputMode::line(int p_line) const {
  auto block = document()->findBlockByNumber(p_line);
  return block.text();
}

QTextDocument *TextEditInputMode::document() const { return m_textEdit->document(); }

void TextEditInputMode::updateCursor(int p_line, int p_column) {
  auto block = document()->findBlockByNumber(p_line);
...
int TextEditInputMode::toVirtualColumn(int p_line, int p_column, int p_tabWidth) const {
  if (p_column < 0) {
    return 0;
  }
...
int TextEditInputMode::fromVirtualColumn(int p_line, int p_virtualColumn, int p_tabWidth) const {
...
int TextEditInputMode::endLine() const {
  return TextEditUtils::lastVisibleBlock(m_textEdit).blockNumber();
}

int TextEditInputMode::linesDisplayed() {
...
KateViI::Cursor TextEditInputMode::goVisualLineUpDownDry(int p_lines, bool &p_succeed) {
  p_succeed = true;
  if (p_lines == 0) {
...
KateViI::Cursor TextEditInputMode::toKateViCursor(const QTextCursor &p_cursor) const {
  return KateViI::Cursor(p_cursor.block().blockNumber(), p_cursor.positionInBlock());
}

KateViI::Cursor TextEditInputMode::toKateViCursor(int p_position) const {
...
QChar TextEditInputMode::characterAt(const KateViI::Cursor &p_cursor) const {
...
int TextEditInputMode::lineToVisibleLine(int p_line) const { return p_line; }

int TextEditInputMode::visibleLineToLine(int p_line) const { return p_line; }

bool TextEditInputMode::removeText(const KateViI::Range &p_range, bool p_blockWise) {
  if (isReadOnly()) {
    return false;
  }
...
int TextEditInputMode::kateViCursorToPosition(const KateViI::Cursor &p_cursor) const {
  if (!p_cursor.isValid()) {
    return -1;
  }
...
QTextCursor TextEditInputMode::kateViCursorToTextCursor(const KateViI::Cursor &p_cursor) const {
...
QTextCursor TextEditInputMode::kateViRangeToTextCursor(const KateViI::Range &p_range) const {
  auto cursor = textCursor();
  cursor.clearSelection();
  if (p_range.isValid()) {
...
QStringList TextEditInputMode::textLines(const KateViI::Range &p_range, bool p_blockWise) const {
  QStringList ret;
...
QString TextEditInputMode::getText(const KateViI::Range &p_range, bool p_blockWise) const {
  if (!p_range.isValid()) {
    return QString();
  }
...
int TextEditInputMode::firstChar(int p_line) const {
  auto text = line(p_line);
  return TextUtils::firstNonSpace(text);
}

void TextEditInputMode::joinLines(uint p_first, uint p_last, bool p_trimSpace) {
...
int TextEditInputMode::undoCount() const { return document()->isUndoAvailable() ? 1 : 0; }

void TextEditInputMode::undo() { m_textEdit->undo(); }

int TextEditInputMode::redoCount() const { return document()->isRedoAvailable() ? 1 : 0; }

void TextEditInputMode::redo() { m_textEdit->redo(); }

int TextEditInputMode::lastLine() const { return lines() - 1; }

bool TextEditInputMode::replaceText(const KateViI::Range &p_range, const QString &p_text,
                                    bool p_blockWise) {
  if (p_blockWise) {
    EDITOR_NIY;
  } else {
...
bool TextEditInputMode::selection() const { return m_textEdit->hasSelection(); }

bool TextEditInputMode::insertText(const KateViI::Cursor &p_position, const QString &p_text,
                                   bool p_blockWise) {
  if (isReadOnly()) {
    return false;
  }
...
void TextEditInputMode::connectMarkChanged(
...
void TextEditInputMode::removeMark(int line, uint markType) { EDITOR_NIY; }

void TextEditInputMode::indent(const KateViI::Range &p_range, int p_changes) {
...
void TextEditInputMode::pageDown(bool p_half) {
...
void TextEditInputMode::pageUp(bool p_half) {
  const int blockStep = p_half ? blockCountOfOnePageStep() / 2 : blockCountOfOnePageStep();
  auto cursor = textCursor();
  int block = cursor.block().blockNumber();
...
int TextEditInputMode::blockCountOfOnePageStep() const {
...
void TextEditInputMode::scrollInPage(const KateViI::Cursor &p_pos, KateViI::PagePosition p_dest) {
...
void TextEditInputMode::clearOverriddenSelection() { m_textEdit->clearOverriddenSelection(); }

void TextEditInputMode::backspace() {
...
bool TextEditInputMode::setCursorPosition(KateViI::Cursor p_position) {
...
bool TextEditInputMode::insertLine(int p_line, const QString &p_str) {
  if (isReadOnly()) {
    return false;
  }
...
bool TextEditInputMode::isCompletionActive() const { return false; }

void TextEditInputMode::completionNext(bool p_reversed) { Q_UNUSED(p_reversed); }

// When generating selection items, whether do it reversedly or not.
void TextEditInputMode::userInvokedCompletion(bool p_reversed) { Q_UNUSED(p_reversed); }

void TextEditInputMode::completionExecute() {}

void TextEditInputMode::connectFocusIn(std::function<void()> p_slot) {
  connect(this, &TextEditInputMode::textEditFocusIn, this, p_slot);
}

void TextEditInputMode::connectFocusOut(std::function<void()> p_slot) {
  connect(this, &TextEditInputMode::textEditFocusOut, this, p_slot);
}

QString TextEditInputMode::wordAt(const KateViI::Cursor &cursor) const {
  EDITOR_NIY;
  // TODO: Refer completer.cpp.
  return QString();
}

bool TextEditInputMode::isReadOnly() const { return m_textEdit->isReadOnly(); }

void TextEditInputMode::connectMouseReleased(std::function<void(QMouseEvent *)> p_slot) {
  connect(m_textEdit, &VTextEdit::mouseReleased, this, p_slot);
}

void TextEditInputMode::scrollUp() {
...
void TextEditInputMode::scrollDown() {
  QScrollBar *vbar = m_textEdit->verticalScrollBar();
  if (vbar && (vbar->minimum() != vbar->maximum())) {
...
bool TextEditInputMode::foldAtCursor() { return false; }

bool TextEditInputMode::unfoldAtCursor() { return false; }
